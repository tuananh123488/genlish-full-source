...
import { notifyType } from '@/context/NotifyContext'
...
const Notification = ({ status, message, setInfomation }) => {
    ...
    return (
        <div style={{ top: info.status === 'none' ? '-200px' : '8px', transition: '0.3s', backgroundColor: 'white' }} className='items-center px-3 flex w-[270px] shadow-2xl z-50 rounded-md min-h-[70px] border-[1px] border-[#e1e1e1] fixed left-[50%] translate-x-[-50%]'>
            {info.status === notifyType.SUCCESS ?
                <i className='bx bx-check-circle text-[green] text-[40px]'></i>
                :
                info.status === notifyType.FAIL ?
                    <i className='bx bx-bug-alt text-[40px] text-[red]' ></i>
                    :
            ...
            <i style={{ color: '#999' }} onClick={() => setInfo({ status: 'none', message: '' })} className='bx bx-x absolute top-1 right-1 text-[21px] cursor-pointer' ></i>
        </div>
    )
}
