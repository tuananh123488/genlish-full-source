...
const AIPublic = () => {
    ...
    useEffect(() => {
        api({ type: TypeHTTP.GET, sendToken: false, path: `/user/getById/${id}` })
            .then(res => {
                setUser(res)
            })
    }, [id])
    ...
    return (
        ...
        >
            <section className='w-full h-screen flex overflow-hidden'>
                <Navbar />
                <div className='relative w-[100%] py-[1rem] flex flex-col items-center px-[1rem] h-screen overflow-y-auto'>
                    <MicroPhone type={true} handleSendMessage={handleSendMessage} visibleSend={visibleSend} inputRef={inputRef} setValue={setValue} isRecord={isRecord} setIsRecord={setIsRecord} />
                    {messages.length === 0 ? (
                        <div className='w-[1005] h-[80%] flex flex-col justify-center items-center overflow-auto'>
                            <span>Bạn hãy nói gì đó bằng Tiếng Anh</span>
                        </div>
                    ) : (
                        <div ref={chatRef} className='w-[100%] h-[80%] flex flex-col overflow-auto gap-2'>
                            {messages.map((message, index) => {
                                if (message.speaker === 'ai') {
                                    ...
                                    return <div key={index} className='flex items-center justify-end gap-4'>
                                        ...
                                        <div className='bg-cover h-[45px] w-[45px] rounded-full' style={{ backgroundImage: `url(${user?.avatar})` }} />
                                    </div>
                                }
                            })}
                        </div>
